...
prompt_context() {
  ...
  if [[ "$user" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    prompt_segment ${catppuccin_surface0} default "%(!.%{%F{${catppuccin_yellow}}%}.)%{%F{${catppuccin_pink}}%}>"
  fi
}
...
prompt_git() {
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    if [[ -n $dirty ]]; then
      prompt_segment ${catppuccin_yellow} ${catppuccin_base}
    else
      prompt_segment ${catppuccin_rosewater} ${catppuccin_base}
    fi
...
prompt_dir() {
  prompt_segment ${catppuccin_flamingo} ${catppuccin_base} '%~'
  # echo $(pwd | sed -e "s,^$HOME,~," | sed "s@\(.\)[^/]*/@\1/@g")
}
...
prompt_status() {
  local symbols
  symbols=()
  [[ $RETVAL -ne 0 ]] && symbols+="%{%F{${catppuccin_red}}%}✘"
  [[ $UID -eq 501 ]] && symbols+="%{%F{${catppuccin_pink}}%}⟢"
  [[ $(jobs -l | wc -l) -gt 0 ]] && symbols+="%{%F{cyan}%}⚙"

  [[ -n "$symbols" ]] && prompt_segment ${catppuccin_surface0} default "$symbols"
}
